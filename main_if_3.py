...
class stereoCamera(object):
    def __init__(self):
        self.cam_matrix_l = np.array([[3329.13, 0.00000000e+00, 1971.41],
                                      [0.00000000e+00, 3326.41, 1478.27],
                                      [0.00000000e+00, 0.00000000e+00, 1.00000000e+00]])
        self.cam_matrix_r = np.array([[3329.13, 0.00000000e+00, 1971.41],
                                      [0.00000000e+00, 3326.41, 1478.27],
                                      [0.00000000e+00, 0.00000000e+00, 1.00000000e+00]])
        self.distortion_l = np.array([-0.0194744, 0.0348045, 0.000216763, -0.0012502, -0.0482699])
        self.distortion_r = np.array([-0.0194744, 0.0348045, 0.000216763, -0.0012502, -0.0482699])
        self.R = np.identity(3, dtype=np.float64)
        self.T = np.array([[-65], [0.0], [0.0]], dtype=np.float32)
        self.doffs = 0.0
...
def opencv_SGBM(left_img, right_img, use_wls=False):
    ...
    paramL = {"minDisparity": 0,  # 表示可能的最小视差值。通常为0，但有时校正算法会移动图像，所以参数值也要相应调整
              "numDisparities": 144,  # 表示最大的视差值与最小的视差值之差，这个差值总是大于0。在当前的实现中，这个值必须要能被16整除，越大黑色边缘越多，表示不能计算视差的区域
              "blockSize": blockSize,
    ...
    cv.imshow('dispL_before', cv.resize(dispL_nor, (853,640)))
    #cv.waitKey(0)
...
def creatDepthView(dispL=None, focal_length=3329.13, baseline=0.065):
    dispL[dispL <= 15] = 15 # 根据视差图中的直方图确定，也可以根据数据集中提供的视差范围确定
    dispL[dispL > 150] = 150
...
if __name__ == '__main__':
    imgl = cv.imread('D:/code/pycharm/stereo  visuals/self_depth_generation/pic/left_image_3.jpg')
    imgr = cv.imread('D:/code/pycharm/stereo  visuals/self_depth_generation/pic/right_image_3.jpg')
    ...
    cv.imshow('image', cv.resize(imgl, (853,640)))
    cv.imshow('dispL_after', cv.resize(dispL, (853,640)))
    cv.imshow('depthmap', cv.resize(depthmap, (853,640)))
    ...
    depth_range = [0, 10000]  # 深度范围
    save_point_cloud(points, colors, 'point_cloud3.xyz', depth_range)
