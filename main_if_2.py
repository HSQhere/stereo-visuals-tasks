...
class stereoCamera(object):
    def __init__(self):
        self.cam_matrix_l = np.array([[1.16250321e+03, 0.00000000e+00, 8.02557637e+02],
                                      [0.00000000e+00, 1.16625351e+03, 5.75175366e+02],
                                      [0.00000000e+00, 0.00000000e+00, 1.00000000e+00]])
        self.cam_matrix_r = np.array([[1.16250321e+03, 0.00000000e+00, 8.02557637e+02],
                                      [0.00000000e+00, 1.16625351e+03, 5.75175366e+02],
                                      [0.00000000e+00, 0.00000000e+00, 1.00000000e+00]])
        self.distortion_l = np.array([-0.01941144, -0.06134838, 0.01620116, -0.00086964, -0.03158034])
        self.distortion_r = np.array([-0.01941144, -0.06134838, 0.01620116, -0.00086964, -0.03158034])
        self.R = np.identity(3, dtype=np.float64)
        self.T = np.array([[-100], [0.0], [0.0]], dtype=np.float32)
        self.doffs = 0.0
...
def opencv_SGBM(left_img, right_img, use_wls=False):
    ...
    paramL = {"minDisparity": 0,  # 表示可能的最小视差值。通常为0，但有时校正算法会移动图像，所以参数值也要相应调整
              "numDisparities": 224,  # 表示最大的视差值与最小的视差值之差，这个差值总是大于0。在当前的实现中，这个值必须要能被16整除，越大黑色边缘越多，表示不能计算视差的区域
              "blockSize": blockSize,
...
def creatDepthView(dispL=None, focal_length=1162.5, baseline=0.1):
    dispL[dispL <= 120] = 120 # 根据视差图中的直方图确定，也可以根据数据集中提供的视差范围确定
    dispL[dispL > 200] = 200
...
if __name__ == '__main__':
    imgl = cv.imread('D:/code/pycharm/stereo  visuals/self_depth_generation/pic/left_image_2.jpg')
    imgr = cv.imread('D:/code/pycharm/stereo  visuals/self_depth_generation/pic/right_image_2.jpg')
    ...
    save_point_cloud(points, colors, 'point_cloud2.xyz', depth_range)
