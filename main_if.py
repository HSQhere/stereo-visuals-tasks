...
class stereoCamera(object):
    def __init__(self):
        # 自己标定的参数
        # # 内参
        # self.cam_matrix_l = np.array(  [[1.27227494e+03 0.00000000e+00 8.49083520e+02]
        #                                [0.00000000e+00 1.27412512e+03 6.30590111e+02]
        #                                [0.00000000e+00 0.00000000e+00 1.00000000e+00]])
        # # 左右相机畸变系数:[k1, k2, p1, p2, k3]
        # self.distortion_r = np.array([[ 2.85074221e-01 -1.78844907e+00 -8.87703246e-04 -1.03296108e-03 3.36258030e+00]])
        # # 旋转矩阵
        ...
        self.cam_matrix_l = np.array([[1.27227494e+03, 0.00000000e+00, 8.49083520e+02],
                                     [0.00000000e+00, 1.27412512e+03, 6.30590111e+02],
                                     [0.00000000e+00, 0.00000000e+00, 1.00000000e+00]])
        self.cam_matrix_r = np.array([[1.27227494e+03, 0.00000000e+00, 8.49083520e+02],
                                     [0.00000000e+00, 1.27412512e+03, 6.30590111e+02],
                                     [0.00000000e+00, 0.00000000e+00, 1.00000000e+00]])
        self.distortion_l = np.array([0.285074221, -1.78844907, -0.000887703, -0.00103296, 3.36258030])
        self.distortion_r = np.array([0.285074221, -1.78844907, -0.000887703, -0.00103296, 3.36258030])
        self.R = np.eye(3, dtype=np.float32)
        self.T = np.array([[-0.2], [0.0], [0.0]], dtype=np.float32)
        self.doffs = 0.0
...
def opencv_SGBM(left_img, right_img, use_wls=False):
    ...
    paramL = {"minDisparity": 0,  # 表示可能的最小视差值。通常为0，但有时校正算法会移动图像，所以参数值也要相应调整
              "numDisparities": 64,  # 表示最大的视差值与最小的视差值之差，这个差值总是大于0。在当前的实现中，这个值必须要能被16整除，越大黑色边缘越多，表示不能计算视差的区域
              "blockSize": blockSize,
              "P1": 8 * 3 * blockSize * blockSize,  # 控制视差图平滑度的第一个参数 2,904
              "P2": 32 * 3 * blockSize * blockSize,  # 控制视差图平滑度的第二个参数，值越大，视差图越平滑。P1是邻近像素间视差值变化为1时的惩罚值，11,616
              # p2是邻近像素间视差值变化大于1时的惩罚值。算法要求P2>P1,stereo_match.cpp样例中给出一些p1和p2的合理取值。
              "disp12MaxDiff": 1,  # 表示在左右视图检查中最大允许的偏差（整数像素单位）。设为非正值将不做检查。
              "uniquenessRatio": 5,  # 表示由代价函数计算得到的最好（最小）结果值比第二好的值小多少（用百分比表示）才被认为是正确的。通常在5-15之间。
              "speckleWindowSize": 100,  # 表示平滑视差区域的最大窗口尺寸，以考虑噪声斑点或无效性。将它设为0就不会进行斑点过滤，否则应取50-200之间的某个值。
              "speckleRange": 2,  # 指每个已连接部分的最大视差变化，如果进行斑点过滤，则该参数取正值，函数会自动乘以16、一般情况下取1或2就足够了。
              "preFilterCap": 63,
    ...
    cv.imshow('dispL_before', cv.resize(dispL_nor, (640,853)))
    # cv.waitKey(0)
...
def creatDepthView(dispL=None, focal_length=0, baseline=0):
    dispL[dispL < 50] = 50  # 根据视差图中的直方图确定，也可以根据数据集中提供的视差范围确定
    dispL[dispL > 150] = 150
...
if __name__ == '__main__':
    imgl = cv.imread('D:/code/pycharm/stereo  visuals/self_depth_generation/pic/left_image.jpg')
    imgr = cv.imread('D:/code/pycharm/stereo  visuals/self_depth_generation/pic/right_image.jpg')
    ...
    # 深度图
    depthmap = creatDepthView(dispL, 1272, 0.2)

    # 深度图空洞填充
    # depthmap = insert_depth_filled(depthmap)

    # show original depthMap distanceMap
    cv.imshow('image', cv.resize(imgl, (640,853)))
    cv.imshow('dispL_after', cv.resize(dispL, (640,853)))
    cv.imshow('depthmap', cv.resize(depthmap, (640,853)))
