...
class stereoCamera(object):
    def __init__(self):
        ...
        self.T = np.array([[-200], [0.0], [0.0]], dtype=np.float32)
        self.doffs = 0.0
...
def opencv_SGBM(left_img, right_img, use_wls=False):
    ...
    paramL = {"minDisparity": 0,  # 表示可能的最小视差值。通常为0，但有时校正算法会移动图像，所以参数值也要相应调整
              "numDisparities": 160,  # 表示最大的视差值与最小的视差值之差，这个差值总是大于0。在当前的实现中，这个值必须要能被16整除，越大黑色边缘越多，表示不能计算视差的区域
              "blockSize": blockSize,
...
def creatDepthView(dispL=None, focal_length=1272.27, baseline=0.2):
    dispL[dispL <= 20] = 20 # 根据视差图中的直方图确定，也可以根据数据集中提供的视差范围确定
    dispL[dispL > 120] = 120
